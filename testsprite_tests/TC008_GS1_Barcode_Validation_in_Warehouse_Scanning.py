...
async def run_test():
    ...
    try:
        ...
        # -> Input Operative ID and Security Key with provided credentials and submit login form.
        frame = context.pages[-1]
        ...
        # -> Navigate to the Scanning page to perform barcode scanning tests.
        frame = context.pages[-1]
        # Click on 'Scanning' in the Operations menu to go to barcode scanning UI.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Invalid GS1 Barcode Detected').first).to_be_visible(timeout=30000)
        except AssertionError:
            raise AssertionError("Test failed: The barcode scanning system did not reject an invalid or malformed GS1 barcode as expected during warehouse operations.")
