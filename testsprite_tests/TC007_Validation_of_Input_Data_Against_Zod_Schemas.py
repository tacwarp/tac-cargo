...
async def run_test():
    ...
    try:
        ...
        # -> Click on 'Shipments' to open shipment creation form for validation testing.
        frame = context.pages[-1]
        # Click on 'Shipments' navigation link
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Invalid Shipment Data Detected').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Shipment creation form did not display validation errors for invalid data as required by the test plan. Client-side and server-side validations using Zod schemas must prevent invalid or malicious data submissions.")
