"""
TC006: Shipment CRUD Operations with Bulk Action Support Test
Tests shipment table with CRUD operations and bulk actions
"""
import asyncio
...
async def run_test():
    ...
    try:
        ...
        # Step 3: Verify Create button exists
        print("Step 3: Checking Create button...")
        create_btn = page.get_by_role("button", name="Create")
        if await create_btn.count() > 0:
            print("✓ Create button found")
        
        # Step 4: Verify table with checkboxes for bulk actions
        print("Step 4: Checking for bulk action support...")
        checkboxes = await page.locator("input[type='checkbox']").count()
        print(f"✓ Found {checkboxes} checkboxes for selection")
        
        # Step 5: Verify table columns
        print("Step 5: Verifying table structure...")
        table_headers = await page.locator("th").count()
        print(f"✓ Found {table_headers} table columns")
        
        print("\n" + "="*50)
        print("✅ TC006: Shipment CRUD & Bulk Actions - PASSED")
        print("="*50)
    
    except Exception as e:
        print(f"\n❌ TC006: Shipment CRUD & Bulk Actions - FAILED")
        print(f"Error: {str(e)}")
