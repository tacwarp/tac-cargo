...
async def run_test():
    ...
    try:
        ...
        # Enter valid username/email in the Operative ID field
        ...
        frame = context.pages[-1]
        # Enter valid password in the Security Key field
        ...
        # --> Assertions to verify final state - successful login redirects to dashboard
        await page.wait_for_url("**/dashboard**", timeout=15000)
        
        # Verify dashboard is visible
        await expect(page.get_by_text("Mission Control")).to_be_visible(timeout=10000)
        await expect(page.get_by_text("Create Shipment").first).to_be_visible(timeout=10000)
        
        print("✅ TC001: User Login with Valid Credentials - PASSED")
        print("  - Login successful, redirected to Mission Control dashboard")
    
    except Exception as e:
        print(f"❌ TC001: User Login with Valid Credentials - FAILED")
        print(f"Error: {str(e)}")
