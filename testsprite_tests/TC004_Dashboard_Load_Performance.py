...
async def run_test():
    ...
    try:
        ...
        # -> Input email and password, then click 'INITIATE SESSION' to login.
        frame = context.pages[-1]
        # Input the email 'admin@tac.app' into the Operative ID field.
        ...
        frame = context.pages[-1]
        # Input the password 'Test@1498' into the Security Key field.
        elem = frame.locator('xpath=html/body/div[2]/div[2]/div[3]/form/div/div[2]/div[2]/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('Test@1498')
        

        frame = context.pages[-1]
        # Click the 'INITIATE SESSION' button to submit the login form.
        ...
        frame = context.pages[-1]
        await expect(frame.locator('text=Mission-critical overview of TAC Cargo logistics').first).to_be_visible(timeout=2000)
