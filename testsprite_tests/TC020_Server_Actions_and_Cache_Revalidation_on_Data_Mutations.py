...
async def run_test():
    ...
    try:
        ...
        # -> Click on 'Sign In' to authenticate with admin credentials.
        ...
        # -> Input valid Operative ID and Security Key, then submit to authenticate.
        frame = context.pages[-1]
        # Input Operative ID (email)
        ...
        # -> Click on 'Shipments' tab to start mutation testing on shipment records.
        frame = context.pages[-1]
        # Click on 'Shipments' tab to access shipment records for mutation testing
        elem = frame.locator('xpath=html/body/div[2]/div/div[2]/div/div[2]/div[2]/ul/li/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click on 'New Shipment' button to create a new shipment record.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Mutation Success! All validations passed and data persisted.').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Server actions did not trigger correct validation, data persistence, or cache revalidation as required by the test plan.")
