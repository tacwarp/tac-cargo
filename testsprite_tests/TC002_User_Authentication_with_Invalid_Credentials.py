...
async def run_test():
    ...
    try:
        ...
        # -> Enter invalid email and password in the Operative ID and Security Key fields.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        # Enter invalid password in Security Key field
        elem = frame.locator('xpath=html/body/div[2]/div[2]/div[3]/form/div/div[2]/div[2]/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('wrongpassword')
        

        # -> Click the 'INITIATE SESSION' button to attempt login with invalid credentials.
        frame = context.pages[-1]
        # Click the 'INITIATE SESSION' button to submit the login form with invalid credentials
        ...
        try:
            ...
            raise AssertionError("Test failed: Login should be rejected with invalid credentials, but 'Login Successful' message was not expected.")
