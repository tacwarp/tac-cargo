...
const CITIES_BY_STATE: Record<string, string[]> = {
  ...
  'Delhi': ['New Delhi', 'Central Delhi', 'East Delhi', 'North Delhi', 'South Delhi', 'West Delhi', 'Dwarka', 'Rohini', 'Pitampura', 'Janakpuri'],
  ...
  'Karnataka': ['Bengaluru', 'Mysuru', 'Hubli', 'Mangalore', 'Belgaum', 'Davangere', 'Bellary', 'Shimoga'],
...
export function CitySelect({
  ...
  const otherCities = allCities.filter(c => !PRIORITY_CITIES.includes(c) && !stateCities.includes(c))
    ...
    <Select value={value} onValueChange={onValueChange} disabled={disabled}>
      ...
      <SelectContent>
        ...
        {state && stateCities.length > 0 && (
          <SelectGroup>
            <SelectLabel className='text-xs text-primary'>{state}</SelectLabel>
        ...
        {!state && (
          <SelectGroup>
            <SelectLabel className='text-xs text-muted-foreground'>Other Cities</SelectLabel>
