  ...
  for (const varName of REQUIRED_VARS) {
    ...
    if (varName === 'NEXT_PUBLIC_SUPABASE_URL') {
      if (!isValidSupabaseUrl(value)) {
        errors.push(`Invalid Supabase URL: ${varName}. Must be a valid Supabase project URL.`)
      }
    }
    ...
    if (varName === 'NEXT_PUBLIC_SUPABASE_ANON_KEY') {
      if (!isNotPlaceholder(value)) {
        errors.push(`Invalid Supabase anon key: ${varName}. Appears to be a placeholder value.`)
      }
      if (value.length < 100) {
        warnings.push(`Supabase anon key seems too short. Expected JWT format.`)
      }
    }
  }
  ...
  if (isProduction) {
    for (const varName of PRODUCTION_REQUIRED_VARS) {
      ...
      if (varName === 'NEXT_PUBLIC_SITE_URL') {
        if (!isValidUrl(value)) {
          errors.push(`Invalid site URL: ${varName}. Must be a valid HTTP/HTTPS URL.`)
        }
        if (value === '*') {
          errors.push(`SECURITY RISK: ${varName} cannot be wildcard (*) in production. This allows CORS from any origin.`)
        }
        if (value.includes('localhost') || value.includes('127.0.0.1')) {
          warnings.push(`${varName} contains localhost. This may be incorrect for production.`)
        }
      }
    }
  }
...
export function validateAndLog(): void {
  ...
  if (!result.valid) {
    ...
    logger.info('Environment validation passed', {
      ...
      varsChecked: REQUIRED_VARS.length + (process.env.NODE_ENV === 'production' ? PRODUCTION_REQUIRED_VARS.length : 0),
    })
  }
}
...
export function getRequiredEnv(varName: string): string {
  ...
  if (!isNotPlaceholder(value)) {
    throw new Error(`Environment variable ${varName} appears to be a placeholder value`)
  }
