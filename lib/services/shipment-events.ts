...
import { sendEmail, generateShipmentCreatedEmail, generateDeliveryConfirmationEmail } from '@/lib/notifications/email'
import { sendSMS, generateShipmentSMS } from '@/lib/notifications/sms'
import { sendWhatsAppTemplate, WHATSAPP_TEMPLATES } from '@/lib/notifications/whatsapp'
...
export async function createShipmentEvent(data: ShipmentEventData): Promise<{ success: boolean; eventId?: string; error?: string }> {
  try {
...
async function sendStatusNotifications(shipment: any, status: ShipmentStatus): Promise<void> {
      ...
      case 'booked':
        ...
        if (shipment.consignee_phone) {
          await sendSMS({
            ...
            message: generateShipmentSMS({ awbNo: shipment.reference, status, trackingUrl }),
          })
