...
export class InvoiceService {
   ...
   */
  static calculateTotals(items: InvoiceItem[], taxRate = 18): {
    subtotal: number
    taxAmount: number
    total: number
  } {
    const subtotal = items.reduce((sum, item) => {
      ...
      const discount = item.discount ? lineTotal.multiply(item.discount / 100) : Currency(0)
  ...
  static async createInvoice(data: CreateInvoiceData): Promise<Invoice> {
    ...
    const { data: invoice, error } = await supabase.rpc('create_invoice_direct', {
  ...
  static async getAnalytics(): Promise<{
    ...
    const totalRevenue = invoices?.reduce((sum, inv) => 
      Currency(sum).add(inv.total_amount || 0).value, 0) ?? 0

    const outstanding = invoices
      ?.filter(inv => inv.status === 'pending')
      .reduce((sum, inv) => Currency(sum).add(inv.total_amount || 0).value, 0) ?? 0
    ...
    const monthlyMap = invoices?.reduce((acc, inv) => {
      ...
      acc[month] = Currency(acc[month] || 0).add(inv.total_amount || 0).value
      return acc
