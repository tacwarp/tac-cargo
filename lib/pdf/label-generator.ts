 ...
 */
export function generateQRCodeDataUrl(awb: string, baseUrl: string = 'https://taccargo.com'): string {
  const trackingUrl = `${baseUrl}/track/${encodeURIComponent(awb)}`
  // Simple QR code placeholder - in production use qrcode library
  return `data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect fill="white" width="100" height="100"/><text x="50" y="55" text-anchor="middle" font-size="8">${awb}</text></svg>`
}
