...
export function generateInvoiceNumber(sequenceNumber?: number): string {
  ...
  const seq = sequenceNumber || (typeof crypto !== 'undefined' 
    ? parseInt(crypto.getRandomValues(new Uint32Array(1))[0].toString().slice(-4))
...
export function generateAWBNumber(sequenceNumber?: number): string {
  // Use crypto for better randomness to reduce collision risk
  const seq = sequenceNumber || (typeof crypto !== 'undefined'
    ? parseInt(crypto.getRandomValues(new Uint32Array(1))[0].toString().slice(0, 6))
...
export function generateTrackingURL(awb: string, baseUrl?: string): string {
  const base = baseUrl || process.env.NEXT_PUBLIC_APP_URL || 'https://taccargo.com'
...
export function generateBarcodeSVG(data: string, options: {
  ...
  if (displayValue) {
    svg += `<text x="${width / 2}" y="${height - 2}" text-anchor="middle" font-family="monospace" font-size="${fontSize}">${data}</text>`
  }
...
function encodeToCode128(data: string): string {
  ...
  for (const char of data) {
    ...
    pattern += String(code % 2).repeat(2) + '0' + String((code >> 1) % 2).repeat(2) + '0' + String((code >> 2) % 2).repeat(2) + '00'
  }
