...
vi.mock('@/lib/rate-limit', () => ({
  checkRateLimit: vi.fn(() => ({ success: true, remaining: 59, resetIn: 60000, limit: 60 })),
...
describe('Tracking API', () => {
  ...
  describe('GET /api/track', () => {
    ...
    it('should return 404 if shipment not found', async () => {
      ...
      const { GET } = await import('@/app/api/track/route')
      const request = new Request('http://localhost:3000/api/track?awb=TAC123456')
      const response = await GET(request)
      const data = await response.json()
    ...
    it('should return shipment data for valid AWB', async () => {
      const mockShipment = {
        ...
        destination_warehouse: { code: 'IMP', city: 'Imphal', state: 'Manipur' },
      }
      ...
      mockSupabase.from.mockImplementation((table: string) => {
        if (table === 'shipments') {
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({
                single: vi.fn(() => Promise.resolve({ data: mockShipment, error: null })),
              })),
            })),
          }
        }
