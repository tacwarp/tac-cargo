...
const TERMINAL_STATUSES = ['DELIVERED', 'CANCELLED', 'RETURNED', 'FAILED'] as const
...
export function useTracking(trackingId?: string) {
    ...
    const fetchData = useCallback(async (signal?: AbortSignal) => {
        ...
        try {
            const res = await fetch(`/api/tracking?trackingId=${encodeURIComponent(trackingId)}`, { signal })
